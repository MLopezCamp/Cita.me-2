...
export default function DoctorCitaPage() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      try {
        const data = await fetch(`${API}/doctor-portal/cita/${id}?doctor_id=${user.id}`);
        if (!data.ok) throw new Error((await data.json().catch(() => ({}))).detail || "Error");
  ...
  async function handleCompletar(e) {
    ...
    try {
      const data = await fetch(
        `${API}/doctor-portal/completar/${id}?doctor_id=${user.id}&notas=${encodeURIComponent(notas)}`,
        { method: "PUT" }
      );
      if (!data.ok) throw new Error((await data.json().catch(() => ({}))).detail || "Error");
      setExito(true);
        ...
        <>
          ...
          {cita.estado === "confirmada" && (
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              ...
              <form onSubmit={handleCompletar} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Comentarios / Diagnóstico</label>
                ...
                <button type="submit" disabled={enviando || !notas.trim()}
          ...
          {cita.estado === "completada" && (
            <div className="bg-brand-50 border border-brand-200 rounded-xl p-6 text-center">
              ...
              {cita.notas && <p className="text-brand-600 text-sm mt-2">&quot;{cita.notas}&quot;</p>}
            </div>
          )}
